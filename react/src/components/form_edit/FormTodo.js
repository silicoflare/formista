...
export default function FormTodo({ data, setData, imports, setImports, keyName, titl, children }) {
    ...
    return (
        <div className="bg-[--three] p-5 w-3/4 space-y-4">
            ...
            <div className="pl-5" id="fontlist">
                {imports.map((x, i) => (
                    <span className="flex flex-row items-center my-2" key={i} id={`font-${i}`}>
                        <input type="text" className="fonts p-2 bg-[--four] text-white text-lg font-mono w-11/12 focus:outline-none focus:ring-2 focus:ring-[--one]" value={x || ''} onChange={(e) => updateElement(i, e.target.value)} />
                        <button className="p-2 w-11 h-11 mx-2 bg-[--two] text-white focus:outline-none focus:ring-2 focus:ring-[--four]" onClick={() => removeElement(i)} title={`Delete ${children}`}>×</button>
                    </span>
                ))}
            </div>
        </div>
